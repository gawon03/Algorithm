...
def bfs(q, blanks):
    ...
    while True:
        ...
        if blanks == 0 or length == 0:
            if blanks == 0: # 모든 빈 칸에 바이러스를 퍼뜨리면 종료
                return time
            ...
            for d in range(4):
                ...
                if 0<= nx < n and 0 <= ny < n:
                    if visited[nx][ny] == -1: # 아직 방문하지 않은 칸에 대해
                        if board[nx][ny] == 0: # case1 : 빈 칸을 만난 경우
                            q.append((nx, ny))
                            visited[nx][ny] = 1
                            blanks -= 1
                        elif board[nx][ny] == 2: # case2 : 비활성된 바이러스를 만난 경우
                            q.append((nx, ny))
                            visited[nx][ny] = 1
...
# 2. 조합(combinations) 사용해서 활성화할 바이러스 선택
...
if res == INF: # 바이러스를 어떻게 놓아도 전체에 퍼뜨릴 수 없는 경우
    print(-1)
else :
    print(res)