...
def solution(jobs):
    ...
    while i < len(jobs): 
        ...
        for job in jobs:             
            if start < job[0] <= now:  # 이전 작업 완료 이후에 요청된 작업만 추가
                heapq.heappush(heap, [job[1], job[0], job[2]])  # (소요 시간, 요청 시각, 작업 번호)
        
        if heap:
            current = heapq.heappop(heap)  # 가장 우선순위가 높은 작업 pop
